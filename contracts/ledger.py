...
owner = "tz1irHUi6yrS3XU3k4fQaSDA53i3HJBxU1TK"
...
class FA12_Error:
    ...
    NotOwner                        = make("NotOwner")
    NotOwnerCandidate               = make("NotOwnerCandidate")
...
class FA12_governor(FA12_core):
    ...
    def transferOwnership(self, params):
        ...
        sp.verify(self.is_owner(sp.sender), FA12_Error.NotOwner)
        self.data.owner_candidate = params

    @sp.entry_point
    def acceptOwnership(self):
        sp.verify(sp.sender == self.data.owner_candidate, FA12_Error.NotOwnerCandidate)
        self.data.owner = sp.sender
        self.data.owner_candidate = sp.self_address
...
class FA12(
    ...
    def __init__(self, admin, owner, owner_candidate, config, token_metadata = None, contract_metadata = None):
        FA12_core.__init__(self, config, paused = False, administrator = admin, owner = owner, owner_candidate = owner)
...
if "templates" not in __name__:
    ...
    def test():
        ...
        owner = sp.test_account("Onwer")
        owner_candidate = sp.test_account("OnwerCandidate")
        ...
        scenario.show([admin, owner, owner_candidate, alice, bob])
        ...
        scenario.h1("Ownership")
        scenario.h2("Can not transfer if now owner")
        c1.transferOwnership(owner_candidate.address).run(sender = alice, valid = False)

        scenario.h2("Can transfer ownership and accept")
        c1.transferOwnership(owner_candidate.address).run(sender = owner, valid = True)
        c1.acceptOwnership().run(sender = owner_candidate, valid = True)
        c1.setAdministrator(bob.address).run(sender = owner_candidate, valid = True)
        c1.mint(address = alice.address, value = 12).run(sender = bob)
        
        scenario.h2("New Administrator")
        view_administrator = Viewer(sp.TAddress)
        scenario += view_administrator
        c1.getAdministrator((sp.unit, view_administrator.typed.target))
        scenario.verify_equal(view_administrator.data.last, sp.some(bob.address))
